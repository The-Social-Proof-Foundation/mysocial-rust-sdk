...
use query_types::DefaultMySocialNameServiceQuery;
use query_types::DefaultMySocialNameServiceQueryArgs;
...
use query_types::ResolveMySocialNameServiceQuery;
use query_types::ResolveMySocialNameServiceQueryArgs;
...
impl Client {
    ...
    /// Get the address for the provided MySocial domain name.
    pub async fn resolve_mysocial_nameservice_to_address(&self, domain: &str) -> Result<Option<Address>> {
        let operation = ResolveMySocialNameServiceQuery::build(ResolveMySocialNameServiceQueryArgs { name: domain });
        ...
        Ok(response
            .data
            .and_then(|d| d.resolve_mysocialnameservice_address)
            .map(|a| a.address))
    }

    /// Get the default MySocial domain name for the provided address.
    pub async fn default_mysocial_nameservice(&self, address: Address) -> Result<Option<String>> {
        let operation = DefaultMySocialNameServiceQuery::build(DefaultMySocialNameServiceQueryArgs { address });
        ...
        Ok(response
            ...
            .and_then(|a| a.default_mysocialnameservice_name))
    }
}
